document.addEventListener('DOMContentLoaded', async (event) => {
    try {
        const response = await fetch(`/software/api/disambiguate/list_software`, {
            method: "GET"
        });
        if (!response.ok) {
...
async function software_dup_displayer(software_name) {
    try {
        const response = await fetch(`/software/api/disambiguate/${software_name}`, {
            method: "GET"
        });
        if (!response.ok) {
            throw new Error(`HTTP error! status: ${response.status}`);
